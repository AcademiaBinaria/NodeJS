var Articulos = (function () {
    ...
    return {
        ...
        guardando: function (articulo) {
            var deferred = Q.defer();
            collection.insert(articulo, function (err, result) {
                if (!err) {
                    console.log(result);
                    deferred.resolve(result);
                } else {
        ...
        guardandoYdevolviendoLista: function (articulo) {
            var deferred = Q.defer();
            collection.insert(articulo, function (err, result) {
                if (!err) {
                    collection.find().toArray(function (err, result) {
    ...
    function connectDB() {
        ...
        // Iniciamos la base de datos con la info que tenmos en memoria
    function iniciar() {
        collection.find().toArray(function (err, result) {
            if (!err) {
                if (result && result[0]) {
